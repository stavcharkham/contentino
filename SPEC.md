...
What we are building, concretely. The reasoning behind every choice here is in `DECISIONS.md`
and is not repeated. If something in this file looks arbitrary, the entry explaining it is
there under the same date.
...
metrics/          the ledger and the baseline assumptions.
```
...
and a person takes it from there. No trend scanning. No journalist or outlet research. No
automatic promotion of a guideline into a skill. The full list, with reasoning, is in
`PLAN.md`.
...
Tracked in `QUESTIONS.md`. Two affect this document: whether the third content type gets
demoed live (which competes with the Google Docs adapter for day 4), and how much of the $50
budget is already spent.

`PRD.md` is still empty. It is a named deliverable in `CLAUDE.md` and covers the problem,
users, scope, non-goals and success metrics, which is a different document from this one.
